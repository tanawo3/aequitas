...
import { getGenLayerClient, GLOBAL_CONTRACT_ADDRESS, GenLayerNetwork } from '../config/chain';
import contractCode from '../../contracts/AequitasContract.py?raw';
...
export interface AgreementState {
  ...
  party_b: string;
  value_locked: number;
  strict_conditions: string;
  state: 'PENDING_RESOLUTION' | 'FINALIZED_PARTY_A_FAVOR' | 'FINALIZED_PARTY_B_FAVOR' | 'FINALIZED_INCONCLUSIVE';
  adjudication_rationale: string;
}

export interface GenTx {
  hash: string;
  type: 'deploy' | 'register_agreement' | 'adjudicate';
  agreement_id?: string;
  ...
  const deployContract = useCallback(async () => {
    ...
    try {
      ...
      if (receipt && deployedAddress) {
          ...
          localStorage.setItem('deployed_contract_address_V2', deployedAddress);
  ...
  const fetchAgreements = useCallback(async () => {
    ...
    while (retries > 0) {
      try {
        ...
        result = await (client as any).readContract({
          ...
          functionName: 'fetch_all_agreements',
          args: []
        });
        break;
      } catch (e: any) {
    ...
    if (result) {
      try {
        ...
        const parsed: AgreementState[] = Object.values(rawData).map((m: any) => typeof m === 'string' ? JSON.parse(m) : m);
        setAgreements(parsed);
      } catch (parseErr: any) {
        console.error("Failed to parse agreements:", parseErr);
        setError("Failed to parse agreements state: " + parseErr.message);
      }
      setIsFetching(false);
      return;
    }
    ...
    if (isNotFound) {
      ...
      setError("Failed to fetch agreements from the active contract: " + errorMsg);
    }
    setIsFetching(false);
  }, [contractAddress, address, network, networkName]);
  
  const registerAgreement = async (agreement_id: string, party_a: string, party_b: string, value_locked: number, strict_conditions: string) => {
      ...
      try {
          ...
          const hash = await (client as any).writeContract({
              ...
              args: [agreement_id, party_a, party_b, value_locked, strict_conditions]
          });
          ...
          console.error("Write contract (register_agreement) failed:", e);
          let errorMsg = e.message || String(e);
          if (errorMsg.includes('Exception:')) {
              errorMsg = errorMsg.split('Exception:')[1].split('\n')[0].trim();
          } else if (errorMsg.includes('UserError:')) {
              errorMsg = errorMsg.split('UserError:')[1].split('\n')[0].trim();
          }
          setError("Failed to send register_agreement transaction: " + errorMsg);
      }
  };

  const adjudicate = async (agreement_id: string, payload_a: string, payload_b: string) => {
      if (!contractAddress) return;
      ...
      try {
          ...
          const hash = await (client as any).writeContract({
              ...
              args: [agreement_id, payload_a, payload_b]
          });
          ...
          console.error("Consensus Transaction (adjudicate) failed:", e);
          let errorMsg = e.message || String(e);
          if (errorMsg.includes('Exception:')) {
              errorMsg = errorMsg.split('Exception:')[1].split('\n')[0].trim();
          } else if (errorMsg.includes('UserError:')) {
          ...
          setError("Consensus execution failed: " + errorMsg);
      } finally {
